...
class SafteyNetTest < MiniTest::Unit::TestCase
  ...
  def test_dont_modify_given_board
    ...
    board = Boards.empty_board
	  cell = WinningMove.new(board).calculate_cell_for_win("X")
...
class WinningMoveTest < MiniTest::Unit::TestCase
  ...
  def test_for_an_empty_board_there_is_none
	  cell = WinningMove.new(Boards.empty_board).calculate_cell_for_win("X")
	  assert_equal cell, nil
  end
  
  def test_fill_cell_0_to_win_for_player_1
	  cell = WinningMove.new(MyBoards.fill_cell_0_to_win("X")).calculate_cell_for_win("X")
	  assert_equal cell, 0
  end
  
  def test_fill_cell_0_to_win_for_player_2
	  cell = WinningMove.new(MyBoards.fill_cell_0_to_win("O")).calculate_cell_for_win("O")
	  assert_equal cell, 0
  end
